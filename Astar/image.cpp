...
HRESULT image::init(int width, int height)
{
	...
	*/
	...
	m_blendImage->width = GetSystemMetrics(SM_CXSCREEN);
	m_blendImage->height = GetSystemMetrics(SM_CYSCREEN);
...
void image::alphaRender(HDC hdc, BYTE alpha)
{
	...
	if (_trans)
	{
		...
		BitBlt(m_blendImage->hMemDC, 0, 0, m_blendImage->width, m_blendImage->height,
			hdc, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN), SRCCOPY);
